...
----------------------------------------------------
USE omopBuild;
DROP PROCEDURE IF EXISTS dbo.wipeEMISLabResults;
GO

CREATE PROCEDURE dbo.wipeEMISLabResults AS
BEGIN
...
SELECT 
	...
	-- IIF(mapped.omopDomainId IS NULL, 'measurement', mapped.omopDomainId) as measurement_domain_id,
	IIF(mapped.omopConceptId IS NULL, 0, mapped.omopConceptId) as measurement_concept_id,
	CAST(used.sourceName as VARCHAR(255)) as measurement_source_value
FROM
(
	SELECT DISTINCT
		rb.batterySynonym_id, --TODO: why cannot use the battery_id in which case we can lose the sample and the battery tables from below
		rt.testsynonym_id,
		...
		CONCAT(ts.original_display_name,' (',bs.from_code,')') collate Latin1_General_CI_AS as sourceName
	FROM
	...
	WHERE sourceDomain = 'urn:ordercomms:code-battery-code'
) mapped
...
------------------------------------------------------
DROP PROCEDURE IF EXISTS dbo.extractBatchEMISLabResults;
GO

CREATE PROCEDURE dbo.extractBatchEMISLabResults AS
BEGIN
	...
	BEGIN
		...
		--TEST DECLARE @size INT; SET @size=10000; DECLARE @minReportId INT; SET @minReportId=5000000 
		...
		WHERE 
			...
			and r.discipline_id NOT IN (2,13,25,26,27,28,29)
			-- and r.discipline_id NOT IN (1,3,12)
			--TEST and r.discipline_id = 2 -- histology
			--TEST and r.discipline_id >= 25 -- radiology
			--TEST and r.discipline_id = 13 -- andrology -- this has a PID implication as one field is partner of
		...
		#tmpBatchReport bat
			...
			INNER JOIN ordercomms_review.dbo.rbattery rqb on bat.report_id=rqb.report_id --rqs.rsample_id = rqb.rsample_id --TODO: can we remove this
			INNER JOIN ordercomms_review.dbo.rtest rqt on (bat.report_id = rqt.report_id and rqt.rbattery_id = rqb.rbattery_id)
	...
	INSERT INTO omopBuild.dbo.IdentifiableNote
		...
		WHERE 
			DATALENGTH(textual_result) > 50
			and r.discipline_id NOT IN (1,12) -- Chem haem, micro and andrology (micro (partly) mapped to meas value) - andrology PID risk
		...
		SET @insertRows = @labNotes+@labResults
		...
		SET @minReportId = --@minReportId+@size
			IIF(@insertRows>0, (SELECT MAX(report_id) from #tmpBatchReport), @minReportId);
...
EXEC dbo.extractBatchEMISLabResults
GO
